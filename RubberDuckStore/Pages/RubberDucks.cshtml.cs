    ...
    public class RubberDucksModel : PageModel
    {
        ...
        private Duck? GetDuckById(int id)
        {
            using (var connection = new SqliteConnection("Data Source=RubberDucks.db"))
            {
                ...
                using (var reader = command.ExecuteReader())
                {
                    // If reader returns a record, then we'll return that new duck from the database
                    if (reader.Read())
                    {
                        return new Duck
                        {
            ...
            // if there was no duck matching the ID in the database return null
            return null;
        } // end get duck helper method
    }


    // Simple model class representing a rubber duck product
    // model class maps to a table in the database so that y9ou can create objects
    // in your application that represent t he intance of an entity (rows in the table)
    // in the database
    public class Duck
    {
        // The duck's attributes match the columns in the duck table
